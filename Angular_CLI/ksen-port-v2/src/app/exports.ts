//services
export { ComponentsDisplayService } from './services/components-display.service';
export { BasketService } from './services/basket.service';
export { ImageService } from './services/image.service';
export { ProductService } from './services/product.service';

//components
export { MainComponent } from './components/pages/main/main.component';
export { DeliveryComponent } from './components/pages/delivery/delivery.component';
export { ContactsComponent } from './components/pages/contacts/contacts.component';
export { BasketComponent } from './components/pages/basket/basket.component';
export { AboutComponent } from './components/pages/about/about.component';

export { BasketLableComponent } from './components/widgets/basket-lable/basket-lable.component';
export { BasketProductItemComponent } from './components/widgets/basket-product-item/basket-product-item.component';
export { BasketProductListComponent } from './components/widgets/basket-product-list/basket-product-list.component';
export { BigLeftImgItemComponent } from './components/widgets/big-left-img-item/big-left-img-item.component';
export { CarouselComponent } from './components/widgets/carousel/carousel.component';
export { CarouselItemComponent } from './components/widgets/carousel/carousel-item/carousel-item.component';
export { CatalogComponent } from './components/widgets/catalog/catalog.component';
export { FooterComponent } from './components/widgets/footer/footer.component';
export { FourLineComponent } from './components/widgets/four-line/four-line.component';
export { FourLineListComponent } from './components/widgets/four-line-list/four-line-list.component';
export { GroupItemComponent } from './components/widgets/group-item/group-item.component';
export { MainNavigationComponent } from './components/widgets/main-navigation/main-navigation.component';
export { MediumItemComponent } from './components/widgets/medium-item/medium-item.component';
export { ProductGroupComponent } from './components/widgets/product-group/product-group.component';

//dataproviders
export { ProductsEndpointDataProvider} from './dataproviders/products/products-endpoint.dataprovider';
export * from './dataproviders/products/products.dataprovider';